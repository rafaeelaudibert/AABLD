...
module CitiesHelper
  def city_monthly_travel_chart_for(city)
    ...
    destination_data = city.user_tickets_as_destination.group_by_month(:created_at, last: 12).count
    ...
    column_chart([
                   {
                     name: 'Viagens de origem',
                     data: source_data
                   },
                   {
                     name: 'Viagens de destino',
  ...
  def city_monthly_value_chart_for(city)
    ...
    source_data = UserTicket.group_by_month(:created_at, last: 12).count
    ...
    options = create_chart_options(title: 'Valor total',
                                   subtitle: 'Agrupado por Mês',
                                   xtitle: 'Mês',
                                   ytitle: 'Valor',
                                   palette: 'palette6',
